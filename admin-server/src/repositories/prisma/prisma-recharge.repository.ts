...
export class PrismaRechargeRepository implements IRechargeRepository {    
    ...
    async findRechargeCardsByTenant(tenantId: string, page: number, limit: number): Promise<{ cards: RechargeCard[]; total: number }> {
    ...
    async findRechargeCardBatchesByTenant(tenantId: string, page: number, limit: number): Promise<{ batches: RechargeCardBatch[]; total: number }> {
    ...
    async findRechargeRecordsByTenant(tenantId: string, page: number, limit: number): Promise<{ records: RechargeRecord[]; total: number }> {
    ...
    async findRechargeRecordsByUser(userId: string, page: number, limit: number): Promise<{ records: RechargeRecord[]; total: number }> {
        const [records, total] = await Promise.all([
            this.prisma.rechargeRecord.findMany({
                where: { userId },
                skip: (page - 1) * limit,
