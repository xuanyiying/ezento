...
export class PrismaCircuitBreakerRepository implements ICircuitBreakerRepository {
    ...
    async findCircuitBreakersByTenant(tenantId: string, page: number, limit: number): Promise<{ breakers: CircuitBreaker[]; total: number }> {
    ...
    // 熔断事件管理
    async findCircuitBreakerEvents(breakerId: string, page: number, limit: number): Promise<{ events: CircuitBreakerEvent[]; total: number }> {
