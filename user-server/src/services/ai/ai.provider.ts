...
import { AIRequestOptions, AIResponse, AIStreamHandler, Message, getAIConfig } from "./ai.interfaces";
...
export interface AIProviderAdapter {
  ...
  executeStreamRequest(messages: Message[], handler: AIStreamHandler, options?: AIRequestOptions): Promise<string>;
}
...
export class AlibabaProviderAdapter implements AIProviderAdapter {
   ...
   * 执行流式AI请求
   */
  async executeStreamRequest(messages: Message[], handler: AIStreamHandler, options: AIRequestOptions = {}): Promise<string> {
    let fullResponse = '';
...
export class AIProviderFactory {
  ...
  static getProvider(): AIProviderAdapter {
    ...
    logger.debug(`使用AI提供商: ${aiConfig.aiModel} (端点: ${aiConfig.aiApiEndpoint}, 模型: ${aiConfig.aiModelName})`);
