...
export class AiService {
    ...
    static async *generateStreamingResponse(
        ...
        try {
            ...
            // 3. 格式化会话历史
            const formattedMessages = this.formatHistoryMessages(messages).filter(
                msg => msg.role !== 'system'
            ); // 移除系统消息，我们将使用新的系统提示
            ...
            // 6. 调用流式AI服务
            for await (const chunk of this.generateStreamChunks(formattedMessages, streamHandler, {
                systemPrompt,
            })) {
                yield chunk;
            }
    ...
    static async processConversationMessage(
        ...
        try {
            ...
            if (config.aiApiStream && chunkCallback) {
                logger.info('流式调用', systemPrompt, messages);
                ...
                // 非流式响应
                logger.info('非流式调用', systemPrompt, messages);
