...
class MedicalApiService {
    ...
    public async getDepartments(params?: DepartmentQueryParams): Promise<DepartmentListResponse> {
        try {
            const response = await this.apiClient.get<ApiResponse<DepartmentListResponse>>('/departments', {
                params: {
    ...
    public async getDepartmentById(id: string): Promise<ThirdPartyDepartment | null> {
        try {
            const response = await this.apiClient.get<ApiResponse<ThirdPartyDepartment>>(`/departments/${id}`);
    ...
    public async getDoctorById(id: string): Promise<ThirdPartyDoctor | null> {
        try {
            const response = await this.apiClient.get<ApiResponse<ThirdPartyDoctor>>(`/doctors/${id}`);
     ...
     * @returns 医生列表
     */
    public async getDoctorsByDepartment(departmentId: string, params?: Omit<DoctorQueryParams, 'departmentId'>): Promise<DoctorListResponse> {
        try {
            const response = await this.apiClient.get<ApiResponse<DoctorListResponse>>(`/departments/${departmentId}/doctors`, {
                params: {
                    status: params?.status || 'active',
                    keyword: params?.keyword,
                    minRating: params?.minRating,
            ...
            if (!response.data.success || !response.data.data) {
                throw new Error(`获取科室医生列表失败: ${response.data.error?.message || '未知错误'}`);
            }
     ...
     * @returns 可预约时间
     */
    public async getDoctorAvailability(doctorId: string, startDate: string, endDate: string): Promise<Array<{date: string, timeSlots: string[]}>> {
        try {
            const response = await this.apiClient.get<ApiResponse<Array<{date: string, timeSlots: string[]}>>>(`/doctors/${doctorId}/availability`, {
                params: {
                    startDate,
                    endDate
                }
            ...
            if (!response.data.success || !response.data.data) {
                throw new Error(`获取医生可预约时间失败: ${response.data.error?.message || '未知错误'}`);
            }
