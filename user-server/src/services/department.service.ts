...
import type { DepartmentDocument } from '../interfaces/department.interface';
/**
 * 科室服务
...
class DepartmentService {
    ...
    ): Promise<{ total: number; departments: DepartmentDocument[] }> {
        try {
    ...
    public static async createDepartment(data: DepartmentCreateData): Promise<DepartmentDocument> {
        try {
            ...
            await this.cacheService.setDepartmentById(department.id, department);
    ...
    private static async syncDepartmentToDatabase(apiDepartment: any): Promise<DepartmentDocument> {
        try {
            ...
            logger.error(
                `同步科室(ID: ${apiDepartment.departmentId})到数据库失败: ${error.message}`
            );
            throw error;
        }
    }
