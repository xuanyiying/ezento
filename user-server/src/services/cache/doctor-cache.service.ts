...
export class DoctorCacheService {
    ...
    public async setDoctorById(id: string, doctor: DoctorDocument): Promise<void> {
        try {
            const cacheKey = `${this.DOCTOR_CACHE_PREFIX}${id}`;
            await this.cacheService.set(cacheKey, JSON.stringify(doctor), this.CACHE_TTL);
        } catch (error: any) {
    ...
    public async getDoctorsByDepartment(
        departmentId: string
    ): Promise<{ total: number; doctors: DoctorDocument[] } | null> {
        try {
     ...
     */
    public async setDoctorsByDepartment(
        departmentId: string,
        data: { total: number; doctors: DoctorDocument[] }
    ): Promise<void> {
        try {
            const cacheKey = `${this.DOCTOR_DEPARTMENT_CACHE_PREFIX}${departmentId}`;
            await this.cacheService.set(cacheKey, JSON.stringify(data), this.CACHE_TTL);
        } catch (error: any) {
